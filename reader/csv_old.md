...
// NewCSVTickReaderForConfig creates a CSV reader from a Config
func NewCSVTickReaderForConfig(config *Config) (*CSVTickReader, error) {
	// Get CSV file path
	filePath := config.CSV.FilePath

	// Create reader with default config
	reader, err := NewCSVTickReader(filePath)
	if err != nil {
		return nil, err
	}
...
// Config is imported from main package
// It's referenced but not imported here to avoid circular dependency
// In actual usage, NewCSVTickReaderForConfig should be in a separate factory file
